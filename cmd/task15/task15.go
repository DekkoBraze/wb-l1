...
1. Возникает опасность того, что, попытавшись модифицировать justString, мы также модифицируем v и не заметим этого
2. В программе мы используем 100 первых символов из строки в 1024 символа (они остаются в памяти, а не очищаются после завершения работы функции)
3. Некоторые символы, которые не умещаются в один байт (н-р, Unicode), могут быть обрезаны и отображаться в новой переменной некорректно
Для того, чтобы решить проблемы 1 и 2, мы можем копировать содержимое слайса с помощью strings.clean().
